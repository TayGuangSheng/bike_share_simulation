...
from ..models import Payment, PaymentStatus, Ride, RideState, User
...
class PaymentService:
    ...
    def request_refund(
        ...
        *,
        ...
        if payment is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Payment not found")
        ride = payment.ride
        if ride is None or ride.user_id != requester.id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Payment not found")
        ...
        log_event(
            ...
            payload={"payment_id": payment.id, "ride_id": payment.ride_id, "user_id": requester.id, "reason": reason},
        )
        return PaymentResult(payment=payment, idempotent_record=None)
    ...
    def refund(self, payment_id: int, *, idempotency: IdempotencyService) -> PaymentResult:
        ...
        if payment.status == PaymentStatus.refunded:
            return PaymentResult(payment=payment, idempotent_record=None)
        if payment.status not in (PaymentStatus.captured, PaymentStatus.refund_pending):
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Payment must be captured before refund")
